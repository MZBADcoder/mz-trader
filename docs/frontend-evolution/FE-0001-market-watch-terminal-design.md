...
- `分时 / Intraday` 作为独立图表模式，前端请求后端 `1m` latest bars，不传 `from` / `to` / `count_back`；后端在交易日返回当天 regular session 分时数据，在非交易日返回上一完整交易日分时数据。
- 分时图使用价格线 + volume 展示，并支持 hover crosshair 查看当前分钟的时间、价格和成交量。
- 分时图横轴在早盘数据较少时只显示少量时间标签，且使用比 K 线更大的最小间距，避免标签集中在左侧重叠。
- 分钟级 resolution（`1m` / `5m` / `15m` / `30m` / `60m`）用于阅读历史分钟 K 线，只展示 K 线与 volume，不展示 BOLL 或 MA 指标；日/周/月等较高周期保留技术指标。
- 分钟级 bars 请求使用显式历史时间范围读取数据库内仍保留的数据，不限制为当前交易日尾部。
- K 线横向间距应保持固定且紧凑，不随返回数据量减少而拉宽；当可用 bars 少于可视槽位时，左侧连续绘制已有 K 线，右侧保留空白。
- 图表内不固定显示 latest close 文本；横轴时间标签需要按最小间距过滤，避免稀疏 K 线场景下文字重叠。
- K 线价格区需要保留右侧纵坐标轴，显示当前可视价格范围的刻度值，且 K 线绘图区应避开该轴区域。
