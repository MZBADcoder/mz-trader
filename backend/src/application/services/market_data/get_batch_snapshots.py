...
class GetBatchSnapshotsService:
    ...
    async def execute(self, *, user_id: str, tickers: list[str]) -> BatchSnapshotsResult:
        ...
        if missing_tickers:
            fallback_snapshots, unresolved_tickers = await self._fetch_missing_snapshots(missing_tickers)
    ...
    async def _fetch_missing_snapshots(self, tickers: list[str]) -> tuple[dict[str, Snapshot], list[str]]:
